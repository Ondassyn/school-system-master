...
Meteor.methods({
  ...
  "KetPet.totalRating": function (academicYear) {
    ...
    recordInRating.forEach((school) => {
      let recordResults = KetPetResults.find({
        academicYear: academicYear,
      ...
      recordResults.forEach((record) => {
        if (record.grade == "7") {
          ...
          if (studentLevel == "Pass with Distinction(B1)")
            ratingRecord.grade7DistB1++;
          if (studentLevel == "Pass with Merit(A2)")
            ratingRecord.grade7MeritA2++;
          ...
          if (studentLevel == "Pass with Distinction(B2)")
            ratingRecord.grade8DistB2++;
          if (studentLevel == "Pass with Merit(B1)")
            ratingRecord.grade8MeritB1++;
