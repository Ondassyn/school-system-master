...
Meteor.methods({
  calculateSchoolRating: function () {
    ...
    _.each(results, (result) => {
      result.total_points =
        (result.ubt_points || 0) +
        (result.seminar_points || 0) +
        (result.meeting_points || 0) +
        (result.outdoor_event_points || 0) +
        (result.indoorEvent_points || 0) +
        (result.pbl_points || 0) +
        (result.olympiad_points || 0) +
        (result.subject_week_points || 0) +
        (result.admin_participate_points || 0);

      SchoolPerformaRatings.update(
        { _id: result._id },
        { $set: { total_points: result.total_points } }
      );
    });
  },
  ...
  deleteEvent: function (event_id) {
    ...
    schoolRating = SchoolPerformaRatings.findOne({
      schoolId: assess.schoolId,
      month: assess.month,
    });
    schoolAnnualRating = SchoolPerformaRatings.findOne({
      schoolId: assess.schoolId,
      month: "annual",
    });
    ...
    if (assess.event == "meeting") {
      ...
      SchoolPerformaRatings.update(
        { schoolId: schoolRating.schoolId, month: schoolRating.month },
        { $set: schoolRating }
      );
      SchoolPerformaRatings.update(
        { schoolId: schoolRating.schoolId, month: "annual" },
        { $set: schoolAnnualRating }
      );
    }
    ...
    _.each(schools, (school) => {
      ...
      _.each(grades, (grade) => {
        ...
        _.each(subjects, (subject) => {
          ...
          let sameRating = OlympiadRatings.findOne({
            academicYear: academicYear,
            schoolId: school.schoolId,
            subjectId: subject.subjectId,
            grade: grade,
          ...
          if (sameRating) {
            OlympiadRatings.update(
              { _id: sameRating._id },
              { $set: olympiadRating }
            );
          } else {
            if (c > 0) OlympiadRatings.insert(olympiadRating);
          }
        });

        let sameTotalRating = OlympiadRatings.findOne({
          academicYear: academicYear,
          schoolId: school.schoolId,
          subjectId: "all",
          grade: grade,
        ...
        if (sameTotalRating) {
          OlympiadRatings.update(
            { _id: sameTotalRating._id },
            { $set: olympiadTotalRating }
          );
        } else {
          if (cTotal > 0) OlympiadRatings.insert(olympiadTotalRating);
        }
      });
      ...
      if (sameGradeTotalRating) {
        OlympiadRatings.update(
          { _id: sameGradeTotalRating._id },
          { $set: olympiadGradeTotalRating }
        );
      } else {
        if (cGrade > 0) OlympiadRatings.insert(olympiadGradeTotalRating);
      }
    ...
    _.each(schools, (school) => {
      ...
      _.each(grades, (grade) => {
        ...
        _.each(subjects, (subject) => {
          ...
          let sameRating = JobaRatings.findOne({
            academicYear: academicYear,
            schoolId: school.schoolId,
            subjectId: subject.subjectId,
            grade: grade,
          ...
          if (sameRating) {
            ...
            if (c > 0) JobaRatings.insert(jobaRating);
          }
        });

        let sameTotalRating = JobaRatings.findOne({
          academicYear: academicYear,
          schoolId: school.schoolId,
          subjectId: "all",
          grade: grade,
        ...
        if (sameTotalRating) {
          ...
          if (cTotal > 0) JobaRatings.insert(jobaTotalRating);
        }
      });

      let sameGradeTotalRating = JobaRatings.findOne({
        academicYear: academicYear,
        schoolId: school.schoolId,
        subjectId: "all",
        grade: "all",
      ...
      if (sameGradeTotalRating) {
        JobaRatings.update(
          { _id: sameGradeTotalRating._id },
          { $set: jobaGradeTotalRating }
        );
      } else {
        if (cGrade > 0) JobaRatings.insert(jobaGradeTotalRating);
      }
