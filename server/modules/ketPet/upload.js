  ...
  if (results) {
    _.each(results, (result) => {
      ...
      if (
        ...
        (!result.ReadingAndWriting && !result.Reading)
      ) {
        return;
      }
      ...
      if (grade == "7") {
        ...
        var total50 =
          (result.ReadingAndWriting ? parseInt(result.ReadingAndWriting) : 0) +
        ...
        var total50 =
          (result.Reading ? parseInt(result.Reading) : 0) +
      ...
      let recordInDb = KetPetResults.findOne({
        academicYear: academicYear,
        studentId: student.studentId,
        schoolId: schoolId,
      });
    ...
    if (grade == "7") {
      ...
      let recordLast = KetPetRatings.findOne({
        academicYear: academicYear,
        schoolId: schoolId,
        examPeriod: examPeriod,
      });
    ...
    let recordInRating = KetPetRatings.findOne({
      academicYear: academicYear,
      schoolId: schoolId,
      examPeriod: examPeriod,
    });
