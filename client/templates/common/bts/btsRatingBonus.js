...
import './btsRatingBonus.html';
import { Meteor } from 'meteor/meteor';
import XLSX from 'xlsx';
Template.btsRatingBonus.onCreated(function(){
    let template = this
...
var schoolArray2 = [];
Template.btsRatingBonus.helpers({
    btsNo() {
...
Template.btsRatingBonus.events({
    ...
    //   "Орыс тілі",  "Қазақcтан тарихы", "Дүние тарихы",
      ...
      for(var i = 0; i < btsStore.length; i++){
        ...
        let total = btsStore[i].totalBonus?btsStore[i].totalBonus.toFixed(2):0;
        let mathematic = btsStore[i].mathematicBonus?btsStore[i].mathematicBonus.toFixed(2):0;
        let kazakh_lang = btsStore[i].kazakh_langBonus?btsStore[i].kazakh_langBonus.toFixed(2):0;
        ...
        // let world_history = btsStore[i].world_historyBonus?btsStore[i].world_historyBonus.toFixed(2):0;
        let geography = btsStore[i].geographyBonus?btsStore[i].geographyBonus.toFixed(2):0;
        let physics = btsStore[i].physicsBonus?btsStore[i].physicsBonus.toFixed(2):0;
        let chemistry = btsStore[i].chemistryBonus?btsStore[i].chemistryBonus.toFixed(2):0;
        let biology = btsStore[i].biologyBonus?btsStore[i].biologyBonus.toFixed(2):0;
      ...
      if(data.length == 1){
        ...
        Meteor.call('download', data, (err, wb) => {
          ...
          let sName = 'BTS-'+btsNo+' bonus rating '+selectedLesson+' '+okuJyly+'.xlsx';
    ...
    'click #sortMath'(event,template) {
        sortMathematic *= (-1)
        Session.set('Sort',{mathematicBonus:sortMathematic});
    },
    'click #sortPhysics'(event,template) {
        sortPhysics *= (-1)
        Session.set('Sort',{physicsBonus:sortPhysics});
    },
    'click #sortChemistry'(event,template) {
        sortChemistry *= (-1)
        Session.set('Sort',{chemistryBonus:sortChemistry});
    },
    'click #sortBio'(event,template) {
        sortBio *= (-1)
        Session.set('Sort',{biologyBonus:sortBio});
    },
    'click #sortHistory'(event,template) {
        sortHistory *= (-1)
        Session.set('Sort',{kazakh_historyBonus:sortHistory});
    },
    'click #sortKazakh'(event,template) {
        sortKazakh *= (-1)
        Session.set('Sort',{kazakh_langBonus:sortKazakh});
    },
    'click #sortTur'(event,template) {
        sortTur *= (-1)
        Session.set('Sort',{turkish_langBonus:sortTur});
    },
    'click #sortRus'(event,template) {
        sortRus *= (-1)
        Session.set('Sort',{russian_langBonus:sortRus});
    },
    'click #sortGeography'(event,template) {
        sortGeography *= (-1)
        Session.set('Sort',{geographyBonus:sortGeography});
    },
    'click #sortWorldHistory'(event,template) {
        sortWorldHistory *= (-1)
        Session.set('Sort',{world_historyBonus:sortWorldHistory});
    },
    'click #sortTotal'(event,template) {
        sortTotal *= (-1)
        Session.set('Sort',{totalBonus:sortTotal});
    },
})
