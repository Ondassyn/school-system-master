...
import "./btsRatingByCategory_2021_2022.html";
import { Meteor } from "meteor/meteor";
import XLSX from "xlsx";
Template.btsRatingByCategory_2021_2022.onCreated(function () {
  let template = this;
...
Template.btsRatingByCategory_2021_2022.helpers({
  filter1() {
    // Жалпы
  ...
  filter8() {
    // Жалпы - Категория А  - Категория B
    return Template.instance().filter.get() == "filter8";
  },
  btsNo() {
  ...
  btsNo1_or_2_or_3() {
    return (
      FlowRouter.getParam("btsNo") == "1" ||
      FlowRouter.getParam("btsNo") == "2" ||
      FlowRouter.getParam("btsNo") == "3"
    );
  },
...
Template.btsRatingByCategory_2021_2022.events({
  ...
  "change #selectFilter"(event, template) {
    ...
    if (filterN == "filter1") {
      ...
      Session.set("Sort", { total: -1 });
    } else if (filterN == "filter8") {
      Session.set("Sort", { totalC: -1 });
    }
  },
  ...
  "click #export"(event, template) {
    ...
    for (var i = 0; i < btsStore.length; i++) {
      ...
      let mektepAty = Schools.findOne({ schoolId: btsStore[i].schoolId })
        ? Schools.findOne({ schoolId: btsStore[i].schoolId }).shortName
        : undefined;
      let mathematic = btsStore[i].mathematic
        ? btsStore[i].mathematic.toFixed(2)
        : 0;
      let kazakh_lang = btsStore[i].kazakh_lang
        ? btsStore[i].kazakh_lang.toFixed(2)
        : 0;
      let geography = btsStore[i].geography
        ? btsStore[i].geography.toFixed(2)
        : 0;
      let physics = btsStore[i].physics ? btsStore[i].physics.toFixed(2) : 0;
      let chemistry = btsStore[i].chemistry
        ? btsStore[i].chemistry.toFixed(2)
        : 0;
      ...
      let mathematicA = btsStore[i].mathematicA
        ? btsStore[i].mathematicA.toFixed(2)
        : 0;
      let kazakh_langA = btsStore[i].kazakh_langA
        ? btsStore[i].kazakh_langA.toFixed(2)
        : 0;
      let geographyA = btsStore[i].geographyA
        ? btsStore[i].geographyA.toFixed(2)
        : 0;
      let physicsA = btsStore[i].physicsA ? btsStore[i].physicsA.toFixed(2) : 0;
      let chemistryA = btsStore[i].chemistryA
        ? btsStore[i].chemistryA.toFixed(2)
        : 0;
      ...
      let mathematicB = btsStore[i].mathematicB
        ? btsStore[i].mathematicB.toFixed(2)
        : 0;
      let kazakh_langB = btsStore[i].kazakh_langB
        ? btsStore[i].kazakh_langB.toFixed(2)
        : 0;
      let geographyB = btsStore[i].geographyB
        ? btsStore[i].geographyB.toFixed(2)
        : 0;
      let physicsB = btsStore[i].physicsB ? btsStore[i].physicsB.toFixed(2) : 0;
      let chemistryB = btsStore[i].chemistryB
        ? btsStore[i].chemistryB.toFixed(2)
        : 0;
  ...
  "click #sortMath"(event, template) {
    if (filterN == "filter1") {
      ...
      Session.set("Sort", { mathematicC: -1 });
    }
  },
  ...
  "click #sortKazakh"(event, template) {
    if (filterN == "filter1") {
      ...
      Session.set("Sort", { kazakh_langC: -1 });
    }
  },
  "click #sortRussian"(event, template) {
    if (filterN == "filter1") {
      Session.set("Sort", { russian_lang: -1 });
    } else if (filterN == "filter2") {
      Session.set("Sort", { russian_langA: -1 });
    } else if (filterN == "filter3") {
      Session.set("Sort", { russian_langB: -1 });
    } else if (filterN == "filter4") {
      Session.set("Sort", { russian_langA: -1 });
    } else if (filterN == "filter5") {
      Session.set("Sort", { russian_langB: -1 });
      ...
      Session.set("Sort", { russian_langC: -1 });
    }
  },
  "click #sortTur"(event, template) {
    if (filterN == "filter1") {
      ...
      Session.set("Sort", { turkish_langC: -1 });
    }
  },
  "click #sortKZHistory"(event, template) {
    if (filterN == "filter1") {
      ...
      Session.set("Sort", { kazakh_historyC: -1 });
    }
  },
  "click #sortWorlHistory"(event, template) {
    if (filterN == "filter1") {
      ...
      Session.set("Sort", { world_historyC: -1 });
    }
  },
