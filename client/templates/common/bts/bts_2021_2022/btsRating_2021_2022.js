...
import "./btsRating_2021_2022.html";
import { Meteor } from "meteor/meteor";
import XLSX from "xlsx";
Template.btsRating_2021_2022.onCreated(function () {
  let template = this;
...
var schoolArray2 = [];
Template.btsRating_2021_2022.helpers({
  totalNumber(schoolId) {
    return BtsResults.find({ schoolId }).count();
  },
  btsNo() {
  ...
  results() {
    ...
    let cursorKboRatings = BtsRatings.find({
  ...
  "click #export"(event, template) {
    ...
    for (var i = 0; i < btsStore.length; i++) {
      ...
      let turkish_lang = btsStore[i].turkish_lang
        ? btsStore[i].turkish_lang.toFixed(2)
        : 0;
      let russian_lang = btsStore[i].russian_lang
        ? btsStore[i].russian_lang.toFixed(2)
        : 0;
      let kazakh_history = btsStore[i].kazakh_history
        ? btsStore[i].kazakh_history.toFixed(2)
        : 0;
