...
import './btsAllResultsBonus.html';
import { Meteor } from 'meteor/meteor';
import XLSX from 'xlsx';
Template.btsAllResultsBonus.onCreated(function(){
    let template = this
...
Template.btsAllResultsBonus.helpers({
    grade7(){
...
Template.btsAllResultsBonus.events({
    ...
    "click #export"(event,template) {
      ...
      if(btsStore.length != 0){
        ...
        if(curGrade == '7'){
          ...
          headers = ["#", "Оқу жылы", "Оқушы ID","Сынып", "Аты Жөні","Жалпы","Математика","География",  "Физика",
            ...
            for(var i = 0; i < btsStore.length; i++){
              ...
              let geography = btsStore[i].geographyBonus?btsStore[i].geographyBonus:'-';
              let physics = btsStore[i].physicsBonus?btsStore[i].physicsBonus:'-';

              let chemistry = btsStore[i].chemistryBonus?btsStore[i].chemistryBonus:'-';
              let biology = btsStore[i].biologyBonus?btsStore[i].biologyBonus:'-';
        ...
        Meteor.call('download', data, (err, wb) => {
          if (err) throw err;
          let sName = 'mektep '+schoolName+' BTS-'+btsNo+' all bonus results grade:'+curGrade+' .xlsx';
