...
Template.schoolStudents.helpers({
  ...
  students() {
    ...
    let surname_search = new RegExp(
      Template.instance().surname_search.get(),
      "i"
    );
    let grade_search = new RegExp(Template.instance().grade_search.get(), "i");
    let division_search = new RegExp(
      Template.instance().division_search.get(),
      "i"
    );
  ...
  levelGroups() {
    return [
      { groupId: "01", name: "Республика" },
      { groupId: "02", name: "Область" },
    ];
  },

  isOlympiadStudent(studentId, subjectId) {
    let student = Students.findOne({ studentId: studentId });
    if (student && student.olympiad == subjectId) return "selected";
  },
  isJobaStudent(studentId, subjectId) {
    let student = Students.findOne({ studentId: studentId });
    if (student && student.joba == subjectId) return "selected";
  },
  ...
  isSelectedLevel(studentId, name) {
    let student = Students.findOne({ studentId: studentId });
    if (student && student.level == name) return "selected";
  },
});
...
Template.schoolStudents.events({
  ...
  "click #export"(event, template) {
    ...
    let olympiadSubjectList = KboCourses.find(
      {},
      { sort: { subjectId: 1 } }
    ).fetch();
    let btsElectiveList = BtsElectiveGroup.find(
      {},
      { sort: { subjectId: 1 } }
    ).fetch();
    ...
    for (var i = 0; i < studentList.length; i++) {
      ...
      let studentBtsElective = btsElectiveList[
        parseInt(studentList[i].electiveGroup)
      ]
        ? btsElectiveList[parseInt(studentList[i].electiveGroup) - 1].name
        : "---";
      var studentOlympiad = "---";
