...
import "./btsUpload_2021_2022.html";

Template.btsUpload_2021_2022.onCreated(function () {
  let template = this;
...
Template.btsUpload_2021_2022.helpers({
  results() {
    return Template.instance().results.get();
  },
});

Template.btsUpload_2021_2022.events({
    ...
    function processData(csv) {
      ...
      for (var i = 0; i < txtlines.length; i++) {
        if (txtlines[i] != "") {
          ...
          let variant = BtsAnswerKeys.findOne({
          ...
          if (!student) {
            ...
            !variant ||
            btsNo != variant.quarter ||
            student.grade != variant.grade
          ) {
            studObj.isValid = false;
            template.errors.set(true);
            alert(
              "Окушының БТС номері, таңдалға күн немесе варианты дұрыс емес \n" +
                studObj.studentId +
                " " +
                studObj.name +
                " " +
            ...
            template.errors.set(true);
            alert(
              "Келесі окушыға күн дұрыс таңдалмады \n" +
                studObj.studentId +
                " " +
                studObj.name +
                " " +
