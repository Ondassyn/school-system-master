...
import './satResults.html';
import XLSX from 'xlsx';

Template.satResults.onCreated(function() {
    let template = this
    ...
    template.autorun(() => {
        template.subscribe("schoolSatResults", template.grade.get());
        template.subscribe("schoolIeltsResults", template.grade.get());
    })
})

Template.satResults.helpers({
    results(){
        let returnList = []; 
...
var saveItem = function(){
  ...
  Meteor.call('Sat.updateSatResults', academicYear.get(), Session.get('editItemId'), Template.instance().grade.get(), editItem);
  Meteor.call('Ielts.updateIeltsResults', academicYear.get(), Session.get('editItemId'), Template.instance().grade.get(), editItem);
  
  Session.set('editItemId', null);
}

Template.satResults.events({
...
Template.satResults.onRendered(function() {
