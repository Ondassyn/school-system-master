...
Template.adminSettings.onCreated(function () {
  ...
  template.subscribe("turkishA1AdminResults", academicYear.get(), "all");
...
Template.adminSettings.helpers({
  ...
  selected(id, num, val) {
    ...
    if (conf) return conf[num] == val ? "selected" : "";
  },
  selectedPeriod() {
    var idd = Template.instance()
      .reportPeriod.get()
      .replace(/[.*+?^${}()|[\]\\]/g, "_");
    let conf = Configs.findOne({ _id: "opeUpload" });

    if (conf) return conf[idd] == "enabled";
...
Template.adminSettings.events({
  ...
  "click #saveCoordinatorAccount"(event, template) {
    ...
    if (confirm("Are u sure?"))
      Meteor.call(
        "resetSchoolCoordinatorPassword",
        this.schoolId,
        passwordTxt,
        (err, res) => {
          if (err) {
  ...
  "click #doMagicStuff"(event, template) {
    ...
    if (confirm("Are you sure you want to do that?")) {
      let teacherAccountsPassword = template.find(
        "[name=teacherAccountsPassword]"
      ).value;
      if (teacherAccountsPassword && teacherAccountsPassword === "777") {
        let ratings = TurkishA1Ratings.find().fetch();
        let results = TurkishA1Results.find().fetch();
        Meteor.call(
          "doMagicStuff",
          academicYear.get(),
          ratings,
  ...
  "change #opePeriodPermission"(event, template) {
    var period = template.reportPeriod
      .get()
      .replace(/[.*+?^${}()|[\]\\]/g, "_");
